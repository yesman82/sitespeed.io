...
var javacrawler = require('./crawler/javacrawler'),
    nodecrawler = require('./crawler/nodecrawler'),
...
function AnalyzeOneSite(config) {
  ...
  this.crawler = config.nodecrawler ? nodecrawler : javacrawler;
}
...
AnalyzeOneSite.prototype._crawl = function(callback) {
  ...
  this.crawler.crawl(this.config.url, this.config, function(err, okUrls, errorUrls) {
    callback(err, okUrls, errorUrls);
  });
};
