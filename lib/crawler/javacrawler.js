...
module.exports.crawl = function(url, config, callback) {
  whereis('java', function searched(err) {
    if (err) {
      callback(new Error('Need Java to crawl urls, make sure it is installed in your $PATH'));
    } else {
      doCrawl(url, config, callback);
    }
  });
};

function doCrawl(url, config, callback) {
  var pUrl, args = [
    '-Xmx' + config.memory + 'm',
    '-Xms' + config.memory + 'm'
  ];
  ...
  crawl.on('close', function(code) {
    ...
    async.parallel([
        ...
        function() {
          return callback(undefined, okUrls, errorUrls);
        });
  });
}
